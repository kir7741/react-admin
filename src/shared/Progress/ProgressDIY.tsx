...
const ProgressDIY = (props: any) => {
  ...
  const percentUtil = useContext(ProgressContext);
  ...
  useEffect(() => {
    ...
    console.log(percentUtil)

    if (!mounted.current) {
      // DidMount
      setPercent(percentUtil.processVal);
      mounted.current = true;
    } else {


      // DidUpdate
      if (percent > percentUtil.processVal) {
        tmTwo.current=setTimeout(() => { setPercent(percent - 1) }, 20);
      }
      else if (percent < percentUtil.processVal) {
        // console.log('update')
        // console.log(percent, percentUtil.processVal)
